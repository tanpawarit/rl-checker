...
def _get_catalog() -> str:
    """Compile once (memoized) — the payload is stable for the process lifetime (restart when the catalog changes)"""
...
def check_ad(
    ...
    contents: list = [_get_catalog(), types.Part.from_bytes(data=image_bytes, mime_type=mime)]
    if text:
